...
export default function ExistingPortfolioTable({tableColumns, selectedAssets, selectedAsset, setSelectedAsset}: Props) {
    ...
    useEffect(() => {
        ...
        setPieChartData((old: ChartableItemType[]) => {
                ...
                selectedAssets.forEach((current: AllocData, key: string) => {
                    ...
                    // let pool = registry.getPoolsContainingToken().getPoolFromSplStringId(current.lp);
                    let pool = registry.getPoolFromLpMint(new PublicKey(current.lp));
                    console.log("pool (2) ", pool);
                    let tmp = {
                        key: key,
                        name: Protocol[current.protocol].charAt(0).toUpperCase() + Protocol[current.protocol].slice(1) + " " + current.lp,
                        value: current.usdcAmount!,  // ((100 * current.weight) / sum),
